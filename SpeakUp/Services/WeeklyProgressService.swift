...
nonisolated struct WeeklyProgressData: Sendable {
    ...
    let dailyScores: [DailyScore]  // day 0=Mon..6=Sun, latest score per day

    var hasImproved: Bool { scoreChange > 0 }
    var sessionsDelta: Int { sessionsThisWeek - sessionsLastWeek }
}

nonisolated struct DailyScore: Sendable {
    let day: Int
    let score: Int
}
    ...
    static func calculate(recordings: [Recording]) -> WeeklyProgressData? {
        ...
        for recording in sortedThisWeek {
            ...
            if let existingIdx = dailyScores.firstIndex(where: { $0.day == weekday }) {
                dailyScores[existingIdx] = DailyScore(day: weekday, score: score)
            } else {
                dailyScores.append(DailyScore(day: weekday, score: score))
            }
        }
