...
7. **The page has no navigation bar.** `HistoryView` hides it (`.toolbar(.hidden, for: .navigationBar)`) like every root tab. Search is an `InlineSearchField` at the top of the scroll — it carries the filter menu on its trailing edge and scrolls away with the content; the `SectionPicker` (Recordings / Progress) is the one pinned row. The bar it replaced said "History" above a tab button labelled History, and `.searchable` hung another ~50pt off it. `ComparisonView` and the pushed recording detail add `.restoresNavigationBar()`. Details: [ui-design-system.md](./ui-design-system.md) rule 9.
