...
11. **Lesson detail is teacher-led.** `LessonBoardHeader` states today's focus + roadmap; `LessonPlanStrip` shows a progress track plus labeled Learn/Practice/Drill/Warm-up/Review chips (icons always visible; completion is a corner badge, never an icon swap); `LessonCoachCue` is a single sparkles cue (no second title row). Activity headers use a trailing Done pill, not a checkmark over glyphs. Bottom bar uses `safeAreaInset` so content clears the sticky CTA. CTAs use pedagogical verbs (`Got it`, `Next · Practice`, `Finish lesson`). Completion restates the objective as "You can now…" plus the worked stages - not a trophy checklist alone.
