...
10. **Hospitality card.** `HospitalityService.evaluateToday` may surface a transient `HospitalityMomentCard` above the modular blocks (same slot grammar as `FriendChallengeCard`). Not a `TodayHomeModule`. See [hospitality.md](./hospitality.md).
