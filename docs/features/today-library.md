...
| Role | Path |
|------|------|
| Views | `SpeakUp/Views/Today/` — `TodayView` (hosts `InteractivePromptCard`), `TodayHomeCustomizeView`, `SessionWordsRow`, `StoryPromptCard`, `WeeklyRecapCard`, `FriendChallengeCard` |
...
11. **Customizable home.** Visible modules and order live in additive `UserSettings.todayHomeLayoutRaw` (`[String]` of `TodayHomeModule` raw values). Empty = factory default via `TodayHomeLayout.resolve`. **Session is always forced visible** — it cannot be hidden. Learn is off by default so Today stays a practice surface. Customize from the Today toolbar (`slider.horizontal.3`) or Settings → Today Layout (`TodayHomeCustomizeView`) — **two entry points, not three**: the old inline "Edit" beside Prep tools implied it edited that section when it edits the whole page, and it sat there permanently.

    **The editor shows the page, not a list of its parts.** Blocks render as `TodayModulePreview` — scaled schematics of the real thing (three rings for rings, prompt lines plus the white Start pill for session, four tinted chips for tools), stacked in render order at roughly proportional heights, so the stack reads as the page it edits. Home-screen grammar throughout: everything wiggles to say it is movable, a ⊖ badge drops a block into the **Hidden** tray, ⊕ or a drag back out returns it, and the tray is itself a drop target. Session carries an "Always on" chip and no ⊖.

    Two things that look like choices but are not. Previews are skeleton bars, never the live module views: the editor has to render identically with an empty database, mid-drag, and inside a drag preview. Reorder rides on `draggable`/`dropDestination` rather than a `DragGesture`: the tiles live in a scroll view, and the system drag is the one that does not fight the scroll and auto-scrolls at the edges. Drag is invisible to assistive tech, so every tile also carries Move up / Move down / Hide `accessibilityActions`, and the wiggle is off under Reduce Motion. Pinned in `TodayHomeLayoutTests`.
