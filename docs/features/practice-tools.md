...
| Shared tile | `SpeakUp/Views/Components/ToolTile.swift` — `ToolTileLabel` (Today strip + History Review grid) |
...
7. Library → Tools leads with a one-line caption (not an intro card), then a **2×2 category grid** — icon, title, count/time meta only. Outcome and best-for stay on VoiceOver / the sheet header; the grid must stay scannable like Prompts categories.
...
11. **Explain once, at the surface where the choice is made.** The Library Tools tab renders a compact **category grid**, then embeds the chosen tool in place (`ToolPresentation.embedded`). Searchable across title/outcome/best-for, empty state on no match. Today's prep strip and History's Review grid both use the compact `ToolTileLabel` (`Views/Components/ToolTile.swift`) — icon + short name, no description. Two densities, one copy source (`PracticeToolKind`) — do not hand-roll a third tile dialect.
