...
5. **One line of chrome, not three.** A tool page opens with a single secondary line — `PracticeToolKind.outcome` — then filters, then content. The nav bar already names the page, so an eyebrow label and a purpose card on top of it were the same sentence three times; `ToolPurposeBanner` was deleted, not relocated. `bestFor` is a browsing aid and stays in the Library rows only.
6. Same header grammar on all four list pages, so they read as one family.
