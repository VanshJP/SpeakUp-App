...
3. Review triggers: `strong_result` / `share_completed`. Never ask while dismissing an achievement or coach celebration — care is not a transaction. Rules in `ReviewEligibility`: once per launch, after first result, **not on paywall**, once per version, ≥60 days between asks.
