...
- **Nothing may change the slot's height mid-take.** The slot is `container − topBar − bottomControls`, so anything that grows either one can step the dial down a rung mid-sentence. The coaching cue is a **reserved lane** at the top of `bottomControls` (`RecordingView.coachingCueLane`, `minHeight` 34): the lane is in the layout whether or not a cue is showing, so a cue firing changes nothing about the slot. It is present only when haptic coaching is on — the setting is off by default and is read before the take starts, so the reservation costs the dial nothing for users who never see a cue. Two earlier shapes both failed. A plain row grew the stack ~58pt the moment it fired, shoving the record button down mid-sentence. Replacing it with an `.overlay` positioned by `alignmentGuide(.top) { $0[.bottom] + 12 }` stopped the resize but not the overlap — the capsule still landed on the top of the record button. Transient chrome on a session screen goes in a lane sized for its resting state, never floated over the controls.
