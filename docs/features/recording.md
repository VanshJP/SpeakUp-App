...
2. One `fullScreenCover` (`ContentView.sessionCover`) hosts `CountdownOverlayView` and then `RecordingView` (`isShowingSession` = `showingCountdown || showingRecording`); the countdown crossfades into the recorder in place. The cover zooms out of the launcher when it names a `matchedTransitionSource` (`SessionStartFooter.startZoomID` via `TodayView.sessionZoomSource`; every other launcher leaves it nil and gets the standard presentation). It is `.interactiveDismissDisabled()` — a zoom presentation can otherwise be dragged away mid-take. Friend-challenge links (`source=share`) add countdown chrome and log `practice_start` as `shared_prompt`.
...
- **No session screen writes a dial `diameter`.** The middle of every session screen is a `SessionDialSlot` (`TimerView.swift`): a greedy `GeometryReader` that takes whatever the top and bottom slots left, then draws the dial at `SessionDial.diameter(fittingWidth:)` — 66% of the slot's **width**, clamped 170–260 — stepping down the `SessionDial.ladder` via `ViewThatFits(in: .vertical)` if that does not fit. The old `Spacer() / 200pt dial / Spacer()` sized the dial without reference to the room it had — worst with `WaveformStyle.off`, where the controls collapse to 80pt and the dial sat in a ~400pt gap with ~90pt empty on either side — and at accessibility text sizes on a small phone the same fixed number pushed the record button off the bottom. Countdown, recording, drills and the Recording Look try-on all use the slot.
- **Width sets the size; height only ever shrinks it.** Sizing from `min(width, height)` was wrong and shipped once: the countdown's slot is ~460pt tall and the recording screen's ~260pt (its bottom carries a record button inside up to 220pt of waveform, and `.rings` is the default), so the dial came out at the 260 cap on one screen and ~206 on the very next — a resize at exactly the hand-off the shared slot exists to smooth. Width is the one input both screens share, since both slots are the container minus the same 16pt page inset. **That inset is load-bearing**: a screen that pages differently draws a different dial. It is why `RecordingLookPreview` uses 16 rather than its old 20.
- **The dial is not the only thing in the slot,** and `GeometryReader` does not clip. A framework cue (recording), a mode metric (drills) and a phase label (try-on) share it, which is what the ladder is for: a diameter that ignored them pushed them out over the record button. If you add something to a slot, do not also add a `Spacer` or a fixed height — let the ladder resolve it.
- **Nothing may change the slot's height mid-take.** The slot is `container − topBar − bottomControls`, so anything that grows either one can step the dial down a rung mid-sentence. The coaching cue is a **reserved lane** at the top of `bottomControls` (`RecordingView.coachingCueLane`, a `@ScaledMetric` `minHeight` of 34 relative to `.caption`, with the cue itself clamped to one line at `minimumScaleFactor(0.75)`): the lane is in the layout whether or not a cue is showing, so a cue firing changes nothing about the slot. It is present only when haptic coaching is on — the setting is off by default and is read before the take starts, so the reservation costs the dial nothing for users who never see a cue. The lane height scales with Dynamic Type and the cue cannot wrap, so the reservation is exact on every iPhone: a flat 34 would have been one line at the default size and a clipped one at accessibility sizes, and a cue that can wrap is a lane that can grow mid-take. Two earlier shapes both failed. A plain row grew the stack ~58pt the moment it fired, shoving the record button down mid-sentence. Replacing it with an `.overlay` positioned by `alignmentGuide(.top) { $0[.bottom] + 12 }` stopped the resize but not the overlap — the capsule still landed on the top of the record button. Transient chrome on a session screen goes in a lane sized for its resting state, never floated over the controls.
- The countdown and the recording screen are laid out **slot for slot** — prompt on top, dial slot in the middle, actions along the bottom — so the hand-off moves nothing but the prompt card shrinking. Changing one screen's vertical structure means changing the other.
...
- The take's focus rides on the **prompt card's meta line** ("Personal Growth · Vocal variety"), not in a pill of its own — two stacked capsules were two rows of chrome for one sentence of context. `focusIntentPill` is the fallback for sessions with no prompt card (story practice, free takes). It is **leading-aligned in the top bar's column and tinted `AppColors.primary`**, matching the meta line it stands in for; centred and dimmed, it read as a stray chip floating in an empty screen with no relationship to the words strip under it.
...
- **One canvas for the whole act.** `RecordingView` paints the backdrop once at its root; the recorder, the analyzing gate and `ScoreRevealView` are content-only stages over it (`ScoreRevealView` has no background of its own — previews add one). Each stage used to bring its own canvas, so stop → analyzing → score crossfaded three full-screen backgrounds. Stages hand off with directional scale + opacity transitions, and the waveform ring collapses into the button on stop.
