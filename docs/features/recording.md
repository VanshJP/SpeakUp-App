...
| Word workout strip | `VocabStrip` in `RecordingView.swift` — today's spotlight words as chips inside the top bar, below the prompt card. Live transcription lights a chip (checkmark + success tint) when `VocabMatcher` hears the word; matches are sticky, so they survive the SFSpeech restarts that a pause triggers. `VocabStrip.shown` owns the cut — the matcher listens for exactly the chips on screen. Read-only during the take — no mid-session toggle. |
| Live captions | `LiveCaptionView` + `LiveTakeText` — last ~14 words of the current utterance, above the timer, in a fixed two-line slot so the timer never shifts. Unambiguous hesitations (`um` / `uh`) paint `AppColors.warning`. Not VoiceOver-live (that would chatter); filler count remains the a11y status. |
