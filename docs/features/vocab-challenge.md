...
1. Off by toggle (`vocabChallengeEnabled`). Default on. Settings → Word Workout exposes exactly three things: the master toggle, words per day (1–3), and whether the app teaches words the user does not already track. `vocabChallengeUseBank` / `UseDictionary` / `SpacedReview` are still stored and still honoured by the picker, but nothing writes them anything but `true` — they were knobs with only one sensible answer sitting on the page that manages the words themselves.
