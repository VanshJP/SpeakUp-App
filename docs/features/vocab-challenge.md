...
3a. **…and a word the dictionary cannot define is still addable.** When that query has no dictionary entry on this device (a misspelling, a name, jargon, or no dictionary downloaded), the lookup card is replaced by **Add “x” to your words**, one tap into the bank. The search used to dead-end on "search a single word" right after the reader had. `canDefine` runs once per pass and only for a word that is new (`newWordQuery`), not on every keystroke over a catalog word.
3b. **A refused add says why.** `SettingsViewModel.vocabWordError` (filler, duplicate, blocked) shows under the search field and is announced to VoiceOver; the bank's add used to only buzz. The sheet claims "saved" only when the add was accepted (its `onAdd` returns the refusal, shown in its action bar).
3c. **Full definition is hidden, not disabled,** when the word has no dictionary entry: `GlassButton` has no disabled look, so a disabled one was a dead button. A bank word's row says "Tap to hear it", not "Tap for the definition". It is the one name for the define action across Words and Read Aloud.
3d. **Rows press with `RowPressStyle`.** The open button spans the card and the add control floats over its trailing edge as a sibling overlay (never inside the label); `.plain` gave no sign a tap had landed. The add control is 44×44 and plays no haptic of its own (`addVocabWord` plays success or warning). Rows stay direct children of the `LazyVStack` (rule 2a).
4. Doors: the Words row on Library → Tools (`PracticeHubView.wordsEntryRow`, grouped with the Improve row on one plate, not a fifth tile - see [today-library.md](./today-library.md)), pushed by `WordLibraryRoute`; and a row at the top of Word Lists → Vocab.
