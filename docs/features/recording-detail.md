...
7. Share / More in the nav bar must stay still. Pin the detail (and analyzing) `PageScrollView` with `pinTopEdge: true` so iOS 26's soft top scroll-edge effect cannot re-inset the floating glass. Use `ellipsis`, not `ellipsis.circle` — the bar already draws a glass circle. Toolbar actions read `recording`, not `detailScreenState == .ready`, or Share is a silent no-op while analyzing.
8. Prompt text on a share card and in the share URL is opt-in. Scores-only shares must not put the prompt (or `beat`) on the link. Story sessions may show the title on the card but never encode story body into the URL.
9. `ShareCardSheet` is the one share surface: variant tabs (Scores / Challenge) over a live preview, a `ScoreCardTheme` filter strip, then Save / Copy / Share. `.scores` **must** stay the default selection — the default card carries no prompt. Themes change only the card backdrop (all stay dark, so the hero body needs no per-theme colors) and persist to `UserSettings.shareCardTheme`; the render cache is keyed by variant **and** theme. Save and Copy log `share_complete` too; they are the same intent by another route.
10. Share captions are three lines at most (headline, prompt quote, URL). Explanation belongs on the card or in the sheet's subtitle, not in text the recipient reads.
