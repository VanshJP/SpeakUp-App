...
4c1. **The check-in gate is post-session only.** `offersSelfCheck` asks only when `source == .postSession` (not the first analyzed take, not already dismissed), or after the user taps **Answer quick questions** on the Coaching tab (`reopenedSelfCheck`, which bypasses both rules and clears on hand-over). A take opened from History, a Story, or Learn always opens on its result: `SessionFeedbackGateStore` lives in memory, so gating on it alone re-asked about weeks-old takes after every relaunch, and the button was dead on a user's first take. The gate holds while the take scores and after the score lands (`isStillProcessing` gives the check-in its "See results" exit), so a score arriving mid-answer no longer tears the questions down and restarts them. A take reopened while still scoring shows the scoring status alone.
4c2. `enqueueProcessingIfNeeded` checks the `overallScore` column as well as `coachAnalysis`. The cache is still nil when `.task` runs, and guarding on it alone marked every scored take as processing on open, which flashed the analyzing screen (and the check-in) before the coordinator cleared the flag.
4c3. **Nothing at the top of the page flips.** `TakeComparisonCard`, `NextStepCard`, and the hero's "vs your average" line all read the personal-average snapshot, so they wait for `baselinesLoaded` and arrive together (animated) instead of printing "Your first scored session" or a weakest-subscore next step and then swapping. The snapshot loads before the WPM backfill, which only feeds a chart further down.
...
7. Prompt text on a share card and in the share URL is always visible before it leaves: the preview is the privacy control. One toggle ("Include prompt and link" / "Include story title") turns the challenge card into a scores-only card. Scores-only shares must not put the prompt (or `beat`) on the link. Story sessions may show the title on the card but never encode story body into the URL.
8. `ShareCardSheet` is the one share surface: a live preview, a `ScoreCardTheme` filter strip, the prompt toggle, then Save / Copy / Share, closed by the system close button top leading. **The challenge card is the default** when the take has a prompt or story title (commit 1e53b58: picking a variant asked a question the user had no reason to care about, and it defaulted to the weaker card); takes with neither never see the toggle and get the scores card. Themes change only the card backdrop (all stay dark, so the hero body needs no per-theme colors) and persist to `UserSettings.shareCardTheme`; the render cache is keyed by variant **and** theme. Save and Copy log `share_complete` too; they are the same intent by another route. Detail's toolbar Share is disabled until the take is scored.
...
16. Every play request goes through `startPlayback(of:at:)`. The drawer, the transcript, the filler chips, and the tips all need the same media checks and the same first-listen gate; a second path would drift from it. The gate holds the requested timestamp in `pendingPlaybackTime` rather than dropping it. **Not now** holds for the rest of the launch (`listenBackDeclinedThisLaunch`): the next play request is the answer, not a reason to ask again. `listenBackCount` increments only after `AudioService.play` succeeds, then `AchievementService.shared` rechecks **Brave Listener** with the saved count; missing/iCloud-pending/failed media earns nothing.
...
The card's denominator is the transcriber's word count while the baseline counts tokenizer words. The few percent of drift between them sits far inside the 25% band, so it cannot flip a verdict.
...
**Playback drawer surface.** `PlaybackDrawerContainer` is Liquid Glass (`glassAppearance.glassTint`, top corners 22, run under the home indicator) with no rim stroke - it was an `.ultraThinMaterial` slab with a hairline, the one grey band on a navy page ([ui-design-system.md](./ui-design-system.md) 15d). It rides in `.safeAreaBar(edge: .bottom)`, so content scrolling under it gets the system scroll edge. Every control is a 44pt target with a haptic (grab row, play/pause, ±10 s), and the scrubber is one adjustable VoiceOver element (position "0:12 of 1:04"; swipe up/down skips ten seconds).

## Coaching tab and accessibility

- **One ask per feeling.** The check-in card ("How did that feel?", secondary; the next step owns the page's white CTA) is the only question. The journal row is **Write a journal note** and files once per take: `Story` has no link back to a recording and the schema is additive-only, so the saved flag is a UserDefaults key per recording id. The self-assessment summary prints the dial's own words (Rough / Shaky / Okay / Good / Great, `takeFeelings` in AnalyzingView) - change them together.
- Tips are one `GlassRowGroup` (rows press with `RowPressStyle`, the tip's colour on its `IconChip`), under `GlassSectionHeader("This session")`. Tip and next-step CTAs read `CoachPracticeRoute.actionTitle` - verb-first, tool names from `PracticeToolKind` / `DrillMode` ("Open Read Aloud", "Start Pause Practice · 30s").
- A transcript passage is **one** VoiceOver element (the words as prose, a filler count as its value) with a "Play repeated opening" action per playable opening. Every word used to be its own stop.
- Radar axis labels are buttons ("Clarity, 88, strongest") that open `MetricExplainerSheet`, which closes with the system close button. Label type follows Dynamic Type up to xxLarge; share-card renders stay at the default size.
