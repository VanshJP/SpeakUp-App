...
12b. **The self-check is a wait-shortener, and is designed as one.** The full-screen recorder shows it while the take scores (`AnalyzingView(waitsForScore: true)`). Order: a **Take saved** header (seal, duration, "the hard part's done") so the take is acknowledged before anything is asked; a live **scoring card** (the take's own waveform being scanned by `TakeScanView`, stage copy, and `EstimatedProgressBar`, which eases toward 90 % over a duration-scaled guess and completes only when the score lands); then **one question at a time** in a card that slides between pages (`pageTransition`, step dots to go back). Each answer moves on after `advanceDelay` (450 ms); the last one saves the check-in and slides to a wrap-up that plays the answer back as the setup for the reveal ("Felt good to you. Let's see if the score agrees.") with the tip carousel under it. When the score lands (`analysisReady`, or `isStillProcessing` false after a failure) the recorder hands over by itself after `handOverBeat`; "See your score" / "Skip to results" hand over at once and **keep any answers already given** (skipping used to discard them). The detail screen's fallback gate leaves `waitsForScore` false and hands over on the last answer, as before. Layout rules that stand: the page always scrolls, the bottom bar stacks helper copy above the control, Yes/No carries no polarity captions.
