...
9. **One page inset.** Root tabs use `.pageContentInsets()` (`AppLayout.pageHorizontal` = 16 + bottom 16). Do not hand-roll `.padding(.horizontal, 20)` on a tab — Learn used to, and the stack looked uneven. Chapter stacks use `AppLayout.chapterSpacing` (20); list hubs use `AppLayout.listSpacing` (16).
    ...
    - **What replaced it:** search is `InlineSearchField` at the top of the page's own scroll content, so it scrolls away like Mail's; the section picker (`SectionPicker`) is the one pinned row; the filter / sort / trophy buttons that lived in the bar wear `.headerIconChrome()` and sit next to what they act on (History: on the search row · Library → Prompts: end of the chip row · Library → Stories: end of the folder bar · Learn: `awardsRow`). An inline field must also declare `.scrollDismissesKeyboard(.interactively)` — `.searchable` gave that for free.
