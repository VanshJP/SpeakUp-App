...
5. Motion: `AppMotion` or `.spring(response: 0.3)` / `.easeInOut(duration: 0.2)`. Respect Reduce Motion. Press feedback is `GlassPressStyle` at scale `0.96` (no scale under Reduce Motion). Content that fades in on arrival uses **`.introReveal(delay:)`**, never a hand-rolled `@State opacity = 0` raised in `onAppear` — that pattern's resting state is invisible and it has stranded users on screens whose only button never faded up (gotcha §23).
