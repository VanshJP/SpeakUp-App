...
10a. **Freezes spend automatically — there is no manual unfreeze control.** `RetentionScheduler.refresh` consumes a banked freeze on app open when yesterday was missed and a streak was live. The spend runs **before** the reminder consent and permission gates: it used to sit behind them, and with `dailyReminderEnabled` off by default most users banked freezes that never spent. Only the "freeze used" notification stays gated. The streak sheet (`StreakDetailView`) shows the banked count, days until the next earn, and snowflake markers on covered days so notifications and UI agree.
