...
10a. **Freezes spend automatically — there is no manual unfreeze control.** `RetentionScheduler.refresh` consumes a banked freeze on app open when yesterday was missed and a streak was live. The streak sheet (`StreakDetailView`) shows the banked count, days until the next earn, and snowflake markers on covered days so notifications and UI agree.
