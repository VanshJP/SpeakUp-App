...
6. **No iCloud file work on the main actor.** A finished take is promoted with the async `promoteToICloudIfNeeded` (awaited in `AudioService.stopRecording`), and jobs resolve and probe media through the `nonisolated` `Recording.resolveStoredURL(_:ubiquityContainer:)` and `waitUntilReadable(_:)`. Both used to run on the main thread right as the daemon began uploading the take, and froze the self-check screen for seconds. See gotcha §29. Throwaway takes (drills, voice calibration, dictation) call `stopRecording(promoteToICloud: false)`: they are read once and deleted, and promoting one only starts an upload the delete then chases. Clear All Data resolves and deletes media in a detached task.
