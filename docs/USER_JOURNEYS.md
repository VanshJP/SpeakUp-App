...
| P1 | As a user with a specific topic, I want to find or add it quickly. | Library → Prompts | **Fixed in the sub-screen audit.** Landing search and the difficulty filter used to leave the category grid on screen (typing did nothing); they now show results. An open category rides in the search field as a removable pill, Add saves from the toolbar, and CSV import reports skipped rows. |
| P1 | As a user rehearsing personal material, I want writing, practice, prep, and progress connected. | Library → Stories → Story detail | **Fixed in the sub-screen audit.** Story takes were always one minute, cutting longer scripts off; they now fit the script (`Story.practiceDuration`). A tag filter shows as a clearable chip, and a new story opens on its own page, where Practice lives. |
...
| P1 | As a user who wants structure, I want the app to remember my next lesson. | Learn → Continue → activity → completion | **Fixed in the sub-screen audit.** Finishing a lesson now shows the completion screen (it read as "Done reviewing"), "Next lesson" no longer skips one, and coming back from a review take keeps the step. The recorder shows the lesson task instead of the app-wide focus. |
...
| P2 | As a user proud of progress, I want to share safely. | Result / Progress → share | **Optimized.** The live preview is the privacy control: the challenge card (prompt + link) is the default when the take has a prompt, one toggle turns it into a scores-only card before anything leaves, and challenge links close the acquisition loop. |
