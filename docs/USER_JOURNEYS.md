...
| P1 | As a user working on articulation, I want scripted and custom speech practice. | Library / next step → Read Aloud | **Optimized.** Catalog, saved text, one-off text, TTS, dictionary, and alignment scoring share one tool. |
...
- Post-first-score setup and the app tour happen after demonstrated value and
  remain dismissible.
...
1. Measure first-result forward-action rate by source and weak area. If one
   source underperforms, inspect that handoff before changing the card.
2. Measure skipped-baseline users reaching their first recording. If the new
   framing does not close the gap, restore the guided baseline itself rather
   than adding more explanation.
3. Add route-contract tests around repeat plans (prompt, Story, free practice,
   duration, framework) after extracting that mapping into a pure type.
4. Validate first-score setup plus seven-step tour on a small phone and at
