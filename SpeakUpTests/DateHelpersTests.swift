...
struct StreakCalculationTests {
    ...
    @Test func practicedTodayIsOne() {
        #expect(Date.calculateStreak(from: [Date()]) == 1)
    }
    ...
    @Test func consecutiveDaysAccumulate() {
        let dates = [Date(), daysAgo(1), daysAgo(2), daysAgo(3)]
