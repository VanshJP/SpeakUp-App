...
| Script | Purpose |
|--------|---------|
| `scripts/agent-verify.sh` | Playbook landmine greps + xcodebuild probe |
| `scripts/agent-doc-drift.sh` | Feature index / cited paths / key symbols must exist |
| `scripts/generate-surface-map.sh` | Regenerate `docs/SURFACE_MAP.md` |

CI job `agent-guards` (Ubuntu) runs verify + doc-drift on every PR. `macos-26` still runs `xcodebuild test`.
...
- `agent/skills` used to be a full duplicate of `.agents/skills` and drifted. Aliases are symlinks so that cannot happen again.
- Verify + doc-drift scripts make landmines and stale maps mechanical on Linux cloud agents.
- Golden evals in `agent/evals/` catch routing regressions without needing Simulator.
- Build/test is capability-based: run `xcodebuild` when it exists (local Mac). Linux cloud agents grep + rely on CI (`macos-26`).
