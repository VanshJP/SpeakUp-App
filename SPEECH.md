...
- Live caption tokens (`LiveTakeText.tokens`) and `heardVocabKeys` write only when the slice / set actually changed. Caption hesitations are a small unambiguous set (`um` / `uh`) — context-aware filler counting still goes through `FillerDetectionPipeline`. Vocab chips use `VocabMatcher` against a transcript that survives SFSpeech request restarts.
