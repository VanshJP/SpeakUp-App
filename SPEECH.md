...
- `process(...)`:
  ...
  5. **Fresh path** — unload local LLM (frees memory), then call `speechService.transcribe(audioURL:fillerConfig:preferredTerms:voiceProfile:)`, which runs the fallback chain `SpeechIsolationService.preprocessIfBeneficial` → `WhisperService.transcribe` (bias prompt toward fillers, `DecodingOptions.temperature = 0.0`, `noSpeechThreshold = 0.6`, `compressionRatioThreshold = 2.4`) → **raw-URL retry** → **WhisperService reload + retry** → Apple Speech (`SFSpeechURLRecognitionRequest`, `taskHint = .dictation`, `addsPunctuation = false`, `requiresOnDeviceRecognition = true` always).
...
The coherence enhancement is invoked from `RecordingDetailView.enhanceCoherenceIfNeeded()` after analysis is saved — it does **not** block the first render. It starts from the detail screen's cached full analysis, not a fresh `fullAnalysis` decode. Blend inside `PromptRelevanceService.coherenceScore(transcript:llmService:promptText:)`, which computes the rule-based score in a detached task: the function is `nonisolated async`, so under approachable concurrency it would otherwise run the embedding and tagger passes on its main-actor caller (gotcha §31):
...
- Live counts are committed-plus-live across utterances. `liveWordCount` / `liveFillerCount` bank each utterance when a request ends or when the on-device recognizer starts a request's transcript over after a pause (`RecognitionContinuity`, shared with Read Aloud and dictation), and stay monotonic only within an utterance. They used to be `max(total, this request)`, which undercounted every word and filler after the first pause — the coaching pace cues and every transcription-scored drill read those numbers.
