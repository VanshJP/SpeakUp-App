...
- Live caption tokens (`LiveTakeText.tokens`) and `heardVocabKeys` write only when the slice / set actually changed. Caption hesitations are a small unambiguous set (`um` / `uh`) — context-aware filler counting still goes through `FillerDetectionPipeline`. Vocab chips use `VocabMatcher` against the current utterance only — `heardVocabKeys` is sticky, so a word matched before a pause stays lit without the service retaining the whole take (which would grow the per-word scan O(n²) for no extra match).
